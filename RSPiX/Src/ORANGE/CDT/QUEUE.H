...
#define QNEXT(i) 		(i+1 >= QSIZE ? 0 : i+1)
#define QPREV(i)		(i-1 >= 0 ? i-1 : QSIZE-1)

template <class T, int16_t sSize>
class RQueue
{
public:
...
protected:
  volatile uintptr_t m_sHead;		// volatile since it could change on interrupt!
  volatile uintptr_t m_sTail;		// volatile since it could change on interrupt!
  T*					m_aptQ[sSize + 1];
};
